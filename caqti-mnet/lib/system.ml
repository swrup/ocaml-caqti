...
type 'a impl =
  ...
  | System : Buffer.t * Mnet.TCP.flow -> system impl
...
module Alarm = struct
  ...
  let schedule ~sw ~stdenv:_ t fn =
    (* TODO
       - what to do of this unsigned_to_int failure?
       - check dt_ns in caqti-mirage, looks like its always 0
       - logs: don't use default source *)
    let t = Mtime.to_uint64_ns t |> Int64.unsigned_to_int |> Option.get in
    let t_now = Mkernel.clock_monotonic () in
    let dt_ns = Int.max 0 (t - t_now) in
...
module Net = struct
  ...
  let connect_tcp ~sw:_ ~stdenv sockaddr =
    ...
    | `Tcp (ipaddr, port) ->
      let flow = Mnet.TCP.connect stdenv.state (ipaddr, port) in
